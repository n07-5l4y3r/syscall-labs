...
namespace offset {
    ...
    namespace _HANDLE_TABLE {
        // Data           : this+0x0, Member, Type: unsigned long, NextHandleNeedingPool
        const s_field NextHandleNeedingPool                              = {
            .byte = 0x0       , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x4, Member, Type: long, ExtraInfoPages
        const s_field ExtraInfoPages                                     = {
            .byte = 0x4       , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x8, Member, Type: volatile unsigned __int64, TableCode
        const s_field TableCode                                          = {
            .byte = 0x8       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x10, Member, Type: struct _EPROCESS *, QuotaProcess
        const s_field QuotaProcess                                       = {
            .byte = 0x10      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x18, Member, Type: struct _LIST_ENTRY, HandleTableList
        const s_field HandleTableList                                    = {
            .byte = 0x18      , .bytes = 0x10      , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x28, Member, Type: unsigned long, UniqueProcessId
        const s_field UniqueProcessId                                    = {
            .byte = 0x28      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x2C, Member, Type: unsigned long, Flags
        const s_field Flags                                              = {
            .byte = 0x2c      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this(bf)+0x2C:0x0 len(0x1), Member, Type: unsigned char, StrictFIFO
        const s_field StrictFIFO                                         = {
            .byte = 0x2c      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x1  };
        // Data           : this(bf)+0x2C:0x1 len(0x1), Member, Type: unsigned char, EnableHandleExceptions
        const s_field EnableHandleExceptions                             = {
            .byte = 0x2c      , .bytes = 0x1       , .bit = 0x1 , .bits = 0x1  };
        // Data           : this(bf)+0x2C:0x2 len(0x1), Member, Type: unsigned char, Rundown
        const s_field Rundown                                            = {
            .byte = 0x2c      , .bytes = 0x1       , .bit = 0x2 , .bits = 0x1  };
        // Data           : this(bf)+0x2C:0x3 len(0x1), Member, Type: unsigned char, Duplicated
        const s_field Duplicated                                         = {
            .byte = 0x2c      , .bytes = 0x1       , .bit = 0x3 , .bits = 0x1  };
        // Data           : this(bf)+0x2C:0x4 len(0x1), Member, Type: unsigned char, RaiseUMExceptionOnInvalidHandleClose
        const s_field RaiseUMExceptionOnInvalidHandleClose               = {
            .byte = 0x2c      , .bytes = 0x1       , .bit = 0x4 , .bits = 0x1  };
        // Data           : this+0x30, Member, Type: struct _EX_PUSH_LOCK, HandleContentionEvent
        const s_field HandleContentionEvent                              = {
            .byte = 0x30      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x38, Member, Type: struct _EX_PUSH_LOCK, HandleTableLock
        const s_field HandleTableLock                                    = {
            .byte = 0x38      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x40, Member, Type: struct _HANDLE_TABLE_FREE_LIST[0x1], FreeLists
        const s_field FreeLists                                          = {
            .byte = 0x40      , .bytes = 0x40      , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x40, Member, Type: unsigned char[0x20], ActualEntry
        const s_field ActualEntry                                        = {
            .byte = 0x40      , .bytes = 0x20      , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x60, Member, Type: struct _HANDLE_TRACE_DEBUG_INFO *, DebugInfo
        const s_field DebugInfo                                          = {
            .byte = 0x60      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
    }
    namespace _HANDLE_TABLE_ENTRY {
        // Data           : this+0x0, Member, Type: volatile __int64, VolatileLowValue
        const s_field VolatileLowValue                                   = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x0, Member, Type: __int64, LowValue
        const s_field LowValue                                           = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x0, Member, Type: struct _HANDLE_TABLE_ENTRY_INFO * volatile, InfoTable
        const s_field InfoTable                                          = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x8, Member, Type: __int64, HighValue
        const s_field HighValue                                          = {
            .byte = 0x8       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x8, Member, Type: union _HANDLE_TABLE_ENTRY *, NextFreeHandleEntry
        const s_field NextFreeHandleEntry                                = {
            .byte = 0x8       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x8, Member, Type: struct _EXHANDLE, LeafHandleValue
        const s_field LeafHandleValue                                    = {
            .byte = 0x8       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x0, Member, Type: __int64, RefCountField
        const s_field RefCountField                                      = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this(bf)+0x0:0x0 len(0x1), Member, Type: unsigned __int64, Unlocked
        const s_field Unlocked                                           = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x1  };
        // Data           : this(bf)+0x0:0x1 len(0x10), Member, Type: unsigned __int64, RefCnt
        const s_field RefCnt                                             = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x1 , .bits = 0x10 };
        // Data           : this(bf)+0x0:0x11 len(0x3), Member, Type: unsigned __int64, Attributes
        const s_field Attributes                                         = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x11, .bits = 0x3  };
        // Data           : this(bf)+0x0:0x14 len(0x2C), Member, Type: unsigned __int64, ObjectPointerBits
        const s_field ObjectPointerBits                                  = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x14, .bits = 0x2c };
        // Data           : this(bf)+0x8:0x0 len(0x19), Member, Type: unsigned long, GrantedAccessBits
        const s_field GrantedAccessBits                                  = {
            .byte = 0x8       , .bytes = 0x4       , .bit = 0x0 , .bits = 0x19 };
        // Data           : this(bf)+0x8:0x19 len(0x1), Member, Type: unsigned long, NoRightsUpgrade
        const s_field NoRightsUpgrade                                    = {
            .byte = 0x8       , .bytes = 0x4       , .bit = 0x19, .bits = 0x1  };
        // Data           : this(bf)+0x8:0x1A len(0x6), Member, Type: unsigned long, Spare1
        const s_field Spare1                                             = {
            .byte = 0x8       , .bytes = 0x4       , .bit = 0x1a, .bits = 0x6  };
        // Data           : this+0xC, Member, Type: unsigned long, Spare2
        const s_field Spare2                                             = {
            .byte = 0xc       , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
    }
    namespace _EPROCESS {
    ...
    namespace _OBJECT_TYPE {
        // Data           : this+0x0, Member, Type: struct _LIST_ENTRY, TypeList
        const s_field TypeList                                           = {
            .byte = 0x0       , .bytes = 0x10      , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x10, Member, Type: struct _UNICODE_STRING, Name
        const s_field Name                                               = {
            .byte = 0x10      , .bytes = 0x10      , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x20, Member, Type: void *, DefaultObject
        const s_field DefaultObject                                      = {
            .byte = 0x20      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x28, Member, Type: unsigned char, Index
        const s_field Index                                              = {
            .byte = 0x28      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x2C, Member, Type: unsigned long, TotalNumberOfObjects
        const s_field TotalNumberOfObjects                               = {
            .byte = 0x2c      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x30, Member, Type: unsigned long, TotalNumberOfHandles
        const s_field TotalNumberOfHandles                               = {
            .byte = 0x30      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x34, Member, Type: unsigned long, HighWaterNumberOfObjects
        const s_field HighWaterNumberOfObjects                           = {
            .byte = 0x34      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x38, Member, Type: unsigned long, HighWaterNumberOfHandles
        const s_field HighWaterNumberOfHandles                           = {
            .byte = 0x38      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x40, Member, Type: struct _OBJECT_TYPE_INITIALIZER, TypeInfo
        const s_field TypeInfo                                           = {
            .byte = 0x40      , .bytes = 0x78      , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0xB8, Member, Type: struct _EX_PUSH_LOCK, TypeLock
        const s_field TypeLock                                           = {
            .byte = 0xb8      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0xC0, Member, Type: unsigned long, Key
        const s_field Key                                                = {
            .byte = 0xc0      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0xC8, Member, Type: struct _LIST_ENTRY, CallbackList
        const s_field CallbackList                                       = {
            .byte = 0xc8      , .bytes = 0x10      , .bit = 0x0 , .bits = 0x0  };
    }
    namespace _OBJECT_HEADER {
        // Data           : this+0x0, Member, Type: __int64, PointerCount
        const s_field PointerCount                                       = {
            .byte = 0x0       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x8, Member, Type: __int64, HandleCount
        const s_field HandleCount                                        = {
            .byte = 0x8       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x8, Member, Type: void *, NextToFree
        const s_field NextToFree                                         = {
            .byte = 0x8       , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x10, Member, Type: struct _EX_PUSH_LOCK, Lock
        const s_field Lock                                               = {
            .byte = 0x10      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x18, Member, Type: unsigned char, TypeIndex
        const s_field TypeIndex                                          = {
            .byte = 0x18      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x19, Member, Type: unsigned char, TraceFlags
        const s_field TraceFlags                                         = {
            .byte = 0x19      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this(bf)+0x19:0x0 len(0x1), Member, Type: unsigned char, DbgRefTrace
        const s_field DbgRefTrace                                        = {
            .byte = 0x19      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x1  };
        // Data           : this(bf)+0x19:0x1 len(0x1), Member, Type: unsigned char, DbgTracePermanent
        const s_field DbgTracePermanent                                  = {
            .byte = 0x19      , .bytes = 0x1       , .bit = 0x1 , .bits = 0x1  };
        // Data           : this+0x1A, Member, Type: unsigned char, InfoMask
        const s_field InfoMask                                           = {
            .byte = 0x1a      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x1B, Member, Type: unsigned char, Flags
        const s_field Flags                                              = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this(bf)+0x1B:0x0 len(0x1), Member, Type: unsigned char, NewObject
        const s_field NewObject                                          = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x0 , .bits = 0x1  };
        // Data           : this(bf)+0x1B:0x1 len(0x1), Member, Type: unsigned char, KernelObject
        const s_field KernelObject                                       = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x1 , .bits = 0x1  };
        // Data           : this(bf)+0x1B:0x2 len(0x1), Member, Type: unsigned char, KernelOnlyAccess
        const s_field KernelOnlyAccess                                   = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x2 , .bits = 0x1  };
        // Data           : this(bf)+0x1B:0x3 len(0x1), Member, Type: unsigned char, ExclusiveObject
        const s_field ExclusiveObject                                    = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x3 , .bits = 0x1  };
        // Data           : this(bf)+0x1B:0x4 len(0x1), Member, Type: unsigned char, PermanentObject
        const s_field PermanentObject                                    = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x4 , .bits = 0x1  };
        // Data           : this(bf)+0x1B:0x5 len(0x1), Member, Type: unsigned char, DefaultSecurityQuota
        const s_field DefaultSecurityQuota                               = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x5 , .bits = 0x1  };
        // Data           : this(bf)+0x1B:0x6 len(0x1), Member, Type: unsigned char, SingleHandleEntry
        const s_field SingleHandleEntry                                  = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x6 , .bits = 0x1  };
        // Data           : this(bf)+0x1B:0x7 len(0x1), Member, Type: unsigned char, DeletedInline
        const s_field DeletedInline                                      = {
            .byte = 0x1b      , .bytes = 0x1       , .bit = 0x7 , .bits = 0x1  };
        // Data           : this+0x1C, Member, Type: unsigned long, Reserved
        const s_field Reserved                                           = {
            .byte = 0x1c      , .bytes = 0x4       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x20, Member, Type: struct _OBJECT_CREATE_INFORMATION *, ObjectCreateInfo
        const s_field ObjectCreateInfo                                   = {
            .byte = 0x20      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x20, Member, Type: void *, QuotaBlockCharged
        const s_field QuotaBlockCharged                                  = {
            .byte = 0x20      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x28, Member, Type: void *, SecurityDescriptor
        const s_field SecurityDescriptor                                 = {
            .byte = 0x28      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
        // Data           : this+0x30, Member, Type: struct _QUAD, Body
        const s_field Body                                               = {
            .byte = 0x30      , .bytes = 0x8       , .bit = 0x0 , .bits = 0x0  };
    }
    namespace _PEB {
