...
#pragma warning( disable: 6001 )
	VirtualFree(local_image_base, 0, MEM_RELEASE);
#pragma warning( pop )
	return intel_driver::FreePool(iqvw64e_device_handle, kernel_image_base);
}
...
void kdmapper::RelocateImageByDelta(portable_executable::vec_relocs relocs, const uint64_t delta)
{
	for (const auto& current_reloc : relocs)
	{
		for (auto i = 0u; i < current_reloc.count; ++i)
		{
			...
			const uint16_t get_offset = current_reloc.item[i] & 0xFFF;

			if (type == IMAGE_REL_BASED_DIR64)
				* reinterpret_cast<uint64_t*>(current_reloc.address + get_offset) += delta;
		}
	}
}
